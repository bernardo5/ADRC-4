...
{
  int t;
  while ((k > 0) && node_distance[((*h)->heapdata)[(k - 1) / 2]]>=node_distance[((*h)->heapdata)[k]]) {
#ifdef DEMO
    /* --------------------------------------------------- */
...
void FixDown(Heap ** h, int k, int*node_distance)
{
  ...
  while ((2 * k + 1) < (*h)->n_elements) {
    j = 2 * k + 1;
    if (((j + 1) < (*h)->n_elements) &&
        (node_distance[(*h)->heapdata[j]]>=node_distance[(*h)->heapdata[j + 1]])){
      /* second offspring is greater */
      j++;
    }
    if (node_distance[(*h)->heapdata[k]]<node_distance[(*h)->heapdata[j]]){
